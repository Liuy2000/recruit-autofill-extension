...
- `1.0.8`：改为点击北森按钮真正绑定 `onClick` 的 `phoenix-button__wraper` 内层，并在弹窗中显示不含个人值的字段失败阶段。
- `1.0.7`：根据中冶京诚表单当前加载的北森官方前端代码，改为点击常量选择器的 `RadioUnchecked` 和地区选择器的 `area-icon-RadioUnchecked` 真实事件节点。
- `1.0.6`：对北森 React 单选和地区组件发送完整鼠标事件，支持非标准确定按钮及异步启用状态，并在确认后校验字段是否真正写回。
- `1.0.5`：修复北森民族、户籍弹层只打开却未保存的问题，改为点击 Phoenix 内层单选节点并识别弹层的确定按钮。
- `1.0.4`：支持北森带“确定”按钮的单选面板（如民族），增强异步三级地区选择，并补充第一学历院校类型映射。
